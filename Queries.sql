SELECT p.Account_ID, p.First_Name, p.Last_Name, h.Property_Type, a.Payment_Amount, a.Payment_Type, a.Payment_Status, r.Signing_Date, b.Branch_Name
FROM Person p, Property h, Payment a, Rental_Agreement r, Branch b, Listing l, Guest g
WHERE p.Account_ID = g.Account_ID AND g.Guest_ID = r.Guest_ID AND r.Listing_ID = l.Listing_ID AND l.Property_ID = h.Property_ID AND h.Branch_Name = b.Branch_Name AND r.RA_ID = a.RA_ID 
...
SELECT p.Account_ID, p.First_Name, p.Last_Name, (SELECT m.Branch_Name FROM Manager m WHERE p.Account_ID = m.Manager_ID OR (e.Manager = m.Manager_ID AND e.Account_ID = p.Account_ID)) AS ‘branch name’, e.Salary, m.Salary
FROM Manger m, Employee e, Person p, Branch b 
WHERE (m.Account_ID = p.Account_ID AND m.Salary > 15000) OR (e.Account_ID = p.Account_ID AND e.Salary > 15000)
